...
from .jwt import MFAAwareTokenObtainPairSerializer


class MFAAwareTokenObtainPairView(TokenObtainPairView):
    serializer_class = MFAAwareTokenObtainPairSerializer
from .views import (
    UserRegistrationViewSet, UserProfileViewSet, TerminalViewSet,
...
urlpatterns = [
    path('verify/<slug:document_type>/<int:pk>/', document_verification, name='document-verification'),
    path('auth/token/', MFAAwareTokenObtainPairView.as_view(), name='token_obtain_pair'),
