...
class Terminal(models.Model):
    """A terminal available for assigning terminal representative accounts."""
...
class Inspection(models.Model):
    ...
    signed_by     = models.ForeignKey(User, null=True, blank=True, on_delete=models.SET_NULL, related_name='signed_inspections')
    ...
    inspector_signed_by = models.ForeignKey(User, null=True, blank=True, on_delete=models.SET_NULL, related_name='dip_inspector_signed')
    client_signed_at    = models.DateTimeField(null=True, blank=True)
    client_signed_by    = models.ForeignKey(User, null=True, blank=True, on_delete=models.SET_NULL, related_name='dip_client_signed')
...
class ProductReceiptCertificate(models.Model):
    ...
    source_shore_tank_calculation = models.OneToOneField(
        'ShoreTankCalculation', on_delete=models.SET_NULL, null=True, blank=True,
...
class ServiceRequest(models.Model):
    ...
    # Telephone numbers are personally identifiable information and are not
    # used for database lookups, making them suitable for encryption at rest.
    contact_phone    = EncryptedTextField(blank=True)
    notes            = EncryptedTextField(blank=True)
...
class SamplingForm(models.Model):
    ...
    vessel_name                       = models.CharField(max_length=200)
    product_name                      = models.CharField(max_length=100)
    # Retained for historical records only; new sampling forms do not collect terminal data.
    terminal                          = models.CharField(max_length=200, blank=True)
...
class ShoreTankCalculationItem(models.Model):
    ...
    tank_no = models.CharField(max_length=50, blank=True)
    operation_label = models.CharField(max_length=30, default='L.DISPL / PROV / FINAL')
...
class DocumentSignature(models.Model):
    """Immutable record of each signing event across all document types."""
    ROLE_CHOICES = (
        ('inspector', 'Inspector'),
        ('terminal_representative', 'Terminal Representative'),
        ('admin', 'Admin'),
    ...
    # Who signed
    signer         = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, related_name='document_signatures')
    role           = models.CharField(max_length=30, choices=ROLE_CHOICES)

    # Cryptographic fields
    document_hash      = models.CharField(max_length=64, help_text='SHA-256 of document content at signing time')
    digital_signature  = models.CharField(max_length=64, help_text='HMAC-SHA256(doc_hash|user_id|timestamp|secret)')
    signing_timestamp  = models.CharField(max_length=30, help_text='ISO timestamp string used in signature')
    signature_image    = models.TextField(blank=True, help_text='Base64 signature image used in the signed PDF')
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=['doc_type', 'doc_id'], name='inspections_doc_typ_doc_id_idx'),
        ]

    def __str__(self):
        return f'{self.doc_type} #{self.doc_number} signed by {self.signer_id} ({self.role})'
