...
from .models import (
    UserProfile, Terminal, Tank, Inspection, Seal, Isolation, ActivityLog,
...
# These checks apply to every save path, including direct API calls and imports.
...
def _require_nonblank(data, fields):
    errors = {}
    for field in fields:
...
def _validate_nonnegative(data, fields):
    errors = {}
    for field in fields:
        value = data.get(field)
        if value is not None and (not math.isfinite(value) or value < 0):
            errors[field] = 'Enter a finite value greater than or equal to zero.'
    if errors:
        raise serializers.ValidationError(errors)


def _require_report_items(items, required_fields):
    if not items:
        raise serializers.ValidationError({'items': 'Add at least one report line.'})
    errors = {}
    for index, item in enumerate(items):
        missing = [field for field in required_fields if not str(item.get(field, '')).strip()]
        if missing:
            errors[index] = {field: 'This field is required.' for field in missing}
    if errors:
...
class UserProfileSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = (
            ...
            'company_email', 'date_joined_company', 'mfa_enabled', 'mfa_confirmed_at',
            'created_at', 'updated_at',
        )
        read_only_fields = ('id', 'mfa_enabled', 'mfa_confirmed_at', 'created_at', 'updated_at')
...
class InspectionReportSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        # Ensure the referenced inspection exists and is an integer id when creating
        inspection = data.get('inspection')
        if inspection is None:
            raise serializers.ValidationError({'inspection': 'Inspection reference is required.'})
        return data
...
class InspectionCreateSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        # The model assigns the sequential DIP number after the initial save.
        # Clients must not provide it when creating a ticket.
    ...
    def validate(self, data):
        _require_nonblank(data, ('vessel_name', 'product_name', 'terminal'))
        _validate_nonnegative(data, (
            'dip_reading', 'water_level', 'overall_dip_1_mm', 'overall_dip_2_mm',
            'overall_dip_3_mm', 'product_dip_1_mm', 'product_dip_2_mm',
            'product_dip_3_mm', 'product_volume_1_l', 'product_volume_2_l',
            'product_volume_3_l', 'free_water_volume_1_l', 'free_water_volume_2_l',
            'free_water_volume_3_l', 'specific_gravity_1', 'specific_gravity_2',
            'specific_gravity_3', 'meter_reading_obs', 'meter_reading_at_20',
            'meter_reading_mts',
        ))
        return data
...
class InspectionDetailSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        read_only_fields = (
            'id', 'inspector', 'approved_by', 'client_name', 'approval_date', 'rejection_reason',
            'signing_step', 'inspector_signed_at', 'client_signed_at', 'verified_at',
            'created_at', 'updated_at',
        )
...
class ProductReceiptCertificateListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = (
            'id', 'certificate_number', 'source_shore_tank_calculation', 'vessel_name', 'terminal',
        ...
        read_only_fields = (
            'id', 'certificate_number', 'source_shore_tank_calculation', 'created_by', 'created_by_name',
            'total_weight_tonnage', 'total_volume_liters',
            'is_signed', 'signed_at', 'signed_by_name', 'document_hash',
        ) + SIGNING_STEP_FIELDS + ('created_at', 'issued_at')
...
class ProductReceiptCertificateDetailSerializer(serializers.ModelSerializer):
    ...
    source_shore_tank_signing_step = serializers.CharField(
        source='source_shore_tank_calculation.signing_step', read_only=True, default=''
    )
    created_by_name      = serializers.CharField(source='created_by.get_full_name', read_only=True)
    signed_by_name       = serializers.CharField(source='signed_by.get_full_name',  read_only=True, allow_null=True)
    total_weight_tonnage = serializers.FloatField(read_only=True)
    ...
    class Meta:
        model = ProductReceiptCertificate
        fields = (
            'id', 'certificate_number', 'source_shore_tank_calculation', 'source_shore_tank_signing_step', 'vessel_name', 'terminal',
    ...
    def validate(self, data):
        if not self.instance:
            _require_nonblank(data, ('vessel_name', 'terminal', 'receipt_date', 'receipt_time'))
        if 'quantity_received_through_inlet_flowmeters' in data:
            _validate_nonnegative(data, ('quantity_received_through_inlet_flowmeters',))
        items = data.get('items')
        if items is None:
            if not self.instance:
                _require_report_items(items, ())
            # allow partial updates that don't touch items
            return data
        _require_report_items(items, ('tank_no', 'product_name', 'weight_tonnage', 'volume_liters'))
        for item in items:
            _validate_nonnegative(item, ('weight_tonnage', 'volume_liters'))
        return data
...
class SealIsolationReportDetailSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        if not self.instance:
            _require_nonblank(data, ('vessel_name', 'product_name', 'terminal', 'report_date'))
        entries = data.get('entries')
        if entries is None:
            if not self.instance:
                _require_report_items(entries, ())
            return data
        _require_report_items(entries, ('location', 'seal_number'))
        return data
...
class ShoreTankCalculationItemSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = (
            'id', 'tank', 'tank_name', 'tank_no', 'operation_label',
...
class ShoreTankCalculationDetailSerializer(serializers.ModelSerializer):
    ...
    product_receipt_certificate_id = serializers.IntegerField(source='product_receipt_certificate.id', read_only=True)
    created_by_name               = serializers.CharField(source='created_by.get_full_name', read_only=True)
    signed_by_name                = serializers.CharField(source='signed_by.get_full_name',  read_only=True, allow_null=True)
    ...
    class Meta:
        model = ShoreTankCalculation
        fields = (
            'id', 'calculation_number', 'product_receipt_certificate_id', 'vessel_name', 'product_name',
...
class VesselReportSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        if not self.instance:
            _require_nonblank(data, ('vessel_name', 'terminal', 'discharge_date'))
        _validate_nonnegative(data, ('total_weight_mt', 'total_volume_m3'))
        for field in ('dip_ticket_ids', 'seal_report_ids', 'shore_calc_ids', 'cert_ids'):
            if field in data:
                ids = data[field]
                if not isinstance(ids, list) or len(ids) != len(set(ids)) or not all(isinstance(pk, int) and pk > 0 for pk in ids):
                    raise serializers.ValidationError({field: 'Use a list of unique, valid document IDs.'})
        request = self.context.get('request')
        user = getattr(request, 'user', None)
...
class ProvisionalOuturnReportSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        if not self.instance:
            _require_nonblank(data, ('vessel_name', 'report_date'))
        items = data.get('items')
        if items is None:
            if not self.instance:
                _require_report_items(items, ())
            return data
        _require_report_items(items, ('terminal_name',))
        for item in items:
            _validate_nonnegative(item, ('ship_volume_m3', 'ship_weight_mt', 'shore_volume_m3', 'shore_weight_mt'))
        return data
...
class StockReportSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        if not self.instance:
            _require_nonblank(data, ('report_date',))
        items = data.get('items')
        if items is None:
            if not self.instance:
                _require_report_items(items, ())
            return data
        _require_report_items(items, ('depot_name', 'date', 'product'))
        for item in items:
            _validate_nonnegative(item, (
                'local_ltrs', 'bps_transit_ltrs', 'non_bps_transit_ltrs',
                'mining_ltrs', 'transshipment_ltrs', 'awaiting_outturn_ltrs',
            ))
        return data
...
class SamplingFormSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = SamplingForm
        fields = (
            'id', 'form_number', 'vessel_name', 'product_name',
    ...
    def validate(self, data):
        # basic required-field validation for sampling forms
        if not self.instance:
            _require_nonblank(data, ('vessel_name', 'product_name', 'sampling_date'))
        _validate_nonnegative(data, ('density_observed',))
        if 'number_of_samples' in data:
            try:
                if data.get('number_of_samples') is not None and int(data.get('number_of_samples')) < 1:
                    raise serializers.ValidationError({'number_of_samples': 'Must be at least 1.'})
            except (ValueError, TypeError):
                raise serializers.ValidationError({'number_of_samples': 'Must be an integer.'})
        if 'sample_quantity' in data:
            try:
                if data.get('sample_quantity') is not None and float(data.get('sample_quantity')) <= 0:
                    raise serializers.ValidationError({'sample_quantity': 'Must be greater than zero.'})
            except (ValueError, TypeError):
                raise serializers.ValidationError({'sample_quantity': 'Must be a number.'})
        return data
...
class DocumentSignatureSerializer(serializers.ModelSerializer):
    signer_name = serializers.CharField(source='signer.get_full_name', read_only=True, allow_null=True)

    class Meta:
        model = DocumentSignature
        fields = (
            'id', 'doc_type', 'doc_id', 'doc_number',
            'signer', 'signer_name', 'role',
