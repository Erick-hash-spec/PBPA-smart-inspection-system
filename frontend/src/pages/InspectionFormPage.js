...
const avg = (values) => {
  ...
  const value = nums.reduce((sum, num) => sum + num, 0) / nums.length;
...
export const InspectionFormPage = () => {
  ...
  const loadExisting = async () => {
    try {
      ...
      const d = res.data;
      setFormData({
        tank: d.tank || '', ticket_number: d.ticket_number || '', tank_no: d.tank_no || '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...formData,
        tank: formData.tank ? Number(formData.tank) : null,
        dip_reading: Number(formData.dip_reading || 0),
        temperature: firstNumber([
          formData.tank_temperature_1_c,
          formData.tank_temperature_2_c,
          formData.tank_temperature_3_c,
          formData.sample_temperature_1_c,
          formData.sample_temperature_2_c,
          formData.sample_temperature_3_c,
          formData.temperature,
        ]),
        water_level: Number(formData.water_level || 0),
      };
      if (!payload.tank_no?.trim()) {
        setError('Tank No is required.');
        setLoading(false);
        return;
      }
      ...
      setError(formatApiError(err, `Failed to ${isEdit ? 'update' : 'create'} inspection`));
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        <Section title="Dip Ticket Information">
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            ...
            <div>
              <label className="block text-xs font-semibold text-gray-500 mb-1.5 uppercase tracking-wide">Tank No <span className="text-red-500">*</span></label>
              <input type="text" name="tank_no" value={formData.tank_no} onChange={handleChange} required className={inputCls} placeholder="Enter inspected tank no." />
            </div>
            ...
            <table className="w-full text-sm min-w-[700px]">
              <thead>
                <tr className="bg-gray-50">
                  ...
                  <th className="text-left px-3 py-2 font-semibold text-gray-600">Average</th>
                </tr>
              </thead>
                ...
                {[
                  ...
                  <tr key={key}>
                    ...
                    <td className="px-3 py-2">
                      <div className="min-h-[40px] rounded-lg border border-slate-200 bg-slate-100 px-3 py-2 text-sm font-bold text-slate-700">
                        {avg([1, 2, 3].map(n => formData[`${key}_${n}${suffix}`])) || '-'}
                      </div>
                    </td>
                  </tr>
                ))}
        ...
        <Section title="PBPA Seal Numbers">
          <div className="overflow-x-auto">
            <table className="w-full text-sm min-w-[560px]">
              <thead>
                <tr className="bg-gray-50">
                  <th className="text-left px-3 py-2 font-semibold text-gray-600">PBPA Seal Position</th>
                  <th className="text-left px-3 py-2 font-semibold text-gray-600">PBPA Seal Number</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-50">
                {[
                  ['Outlet valve seal', 'outlet_valve_seal_number'],
                  ['Water valve seal', 'water_valve_seal_number'],
                  ['Other branches seal', 'other_branches_seal_number'],
                ].map(([label, name]) => (
                  <tr key={name}>
                    <td className="px-3 py-2 font-medium text-gray-700">{label}</td>
                    <td className="px-3 py-2">
        ...
        <Section title="Meter Readings">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              <label className="block text-xs font-semibold text-gray-500 mb-1.5 uppercase tracking-wide">OBS</label>
              <input type="number" step="0.001" name="meter_reading_obs" value={formData.meter_reading_obs} onChange={handleChange} className={inputCls} />
            </div>
            <div>
              <label className="block text-xs font-semibold text-gray-500 mb-1.5 uppercase tracking-wide">@20</label>
              <input type="number" step="0.001" name="meter_reading_at_20" value={formData.meter_reading_at_20} onChange={handleChange} className={inputCls} />
            </div>
            <div>
              <label className="block text-xs font-semibold text-gray-500 mb-1.5 uppercase tracking-wide">MTS</label>
              <input type="number" step="0.001" name="meter_reading_mts" value={formData.meter_reading_mts} onChange={handleChange} className={inputCls} />
            </div>
          </div>
        </Section>
