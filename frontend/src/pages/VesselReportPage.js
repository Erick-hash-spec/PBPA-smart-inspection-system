...
import { vesselReportService, submissionService, shoreTankCalculationService, productReceiptCertificateService } from '../services/api';
import { Ship, Plus, Eye, CheckCircle, Clock, Zap, Filter, Calendar, Search, Download, Printer } from 'lucide-react';
...
export const VesselReportListPage = () => {
  ...
  return (
    <div className="p-6 md:p-8 max-w-6xl mx-auto animate-fade-in">
      ...
      {/* Time Filter */}
      <div className="mb-6 bg-white dark:bg-slate-800 rounded-xl border border-gray-100 dark:border-gray-700 p-4 flex items-center gap-3 flex-wrap">
      ...
      {loading ? (
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm dark:shadow-lg border border-gray-100 dark:border-gray-700 p-16 text-center hover-lift">
          ...
          <p className="text-gray-400 dark:text-gray-500 text-sm mt-1 mb-6">{searchQuery ? 'No matching results' : 'Create your first vessel report to get started'}</p>
          <button onClick={() => navigate('/vessel-reports/new')} className="gradient-primary text-white px-6 py-2.5 rounded-xl text-sm font-semibold hover:shadow-lg transition inline-flex items-center gap-2 hover-lift">
            <Plus className="w-4 h-4" /> New Report
          </button>
        </div>
      ) : (
          ...
          {filteredReports.map(r => (
            <div key={r.id} className="bg-gradient-to-br from-blue-50 to-blue-100 dark:from-blue-900/20 dark:to-blue-800/20 rounded-2xl shadow-sm dark:shadow-lg border border-blue-200 dark:border-blue-700 p-6 hover-lift cursor-pointer group" onClick={() => navigate(`/vessel-reports/${r.id}`)}>
              <div className="flex items-start justify-between mb-4">
                ...
                <span className={`px-3 py-1 rounded-full text-xs font-semibold inline-flex items-center gap-1 ${r.status === 'final' ? 'bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-200' : 'bg-amber-100 dark:bg-amber-900/30 text-amber-800 dark:text-amber-200'}`}>
                  {r.status === 'final' ? <><CheckCircle className="w-3 h-3" /> Final</> : <><Clock className="w-3 h-3" /> Draft</>}
                </span>
              </div>
              <div className="mb-3">
              ...
              <div className="flex gap-2 mt-4 pt-4 border-t border-blue-200 dark:border-blue-700">
                <button onClick={(e) => {e.stopPropagation();navigate(`/vessel-reports/${r.id}`);}} className="flex-1 bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 border border-blue-200 dark:border-blue-700 text-xs font-semibold px-3 py-2 rounded-lg hover:bg-blue-200 dark:hover:bg-blue-900/50 transition hover-scale">
                  <Eye className="w-3.5 h-3.5 inline mr-1" /> View
                </button>
                <button onClick={(e) => {e.stopPropagation();handleDownloadPdf(r);}} className="flex-1 bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300 border border-green-200 dark:border-green-700 text-xs font-semibold px-3 py-2 rounded-lg hover:bg-green-200 dark:hover:bg-green-900/50 transition hover-scale">
                  <Download className="w-3.5 h-3.5 inline mr-1" /> PDF
                </button>
                <button onClick={(e) => {e.stopPropagation();handlePrintPdf(r);}} className="flex-1 bg-gray-100 dark:bg-slate-700 text-gray-700 dark:text-gray-200 border border-gray-200 dark:border-gray-600 text-xs font-semibold px-3 py-2 rounded-lg hover:bg-gray-200 dark:hover:bg-slate-600 transition hover-scale">
                  <Printer className="w-3.5 h-3.5 inline mr-1" /> Print
                </button>
                {r.status === 'draft' && (
                  <button onClick={(e) => {e.stopPropagation();navigate(`/vessel-reports/${r.id}/edit`);}} className="flex-1 bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300 border border-amber-200 dark:border-amber-700 text-xs font-semibold px-3 py-2 rounded-lg hover:bg-amber-200 dark:hover:bg-amber-900/50 transition hover-scale">
                    ✏️ Edit
                  </button>
                )}
              </div>
...
export const VesselReportFormPage = () => {
  ...
  const handleAutoFill = async () => {
    const shoreIds  = form.shore_calc_ids;
    const certIds   = form.cert_ids;
    if (shoreIds.length === 0 && certIds.length === 0) {
      setError('Please link at least one Shore Tank Calculation or Certificate first.');
      return;
    }
    setAutoFilling(true);
    setError('');
    try {
      ...
      for (const docId of shoreIds) {
        try {
          const res = await shoreTankCalculationService.getCalculationById(docId);
          const d = res.data;
      ...
      for (const docId of certIds) {
        try {
          const res = await productReceiptCertificateService.getCertificateById(docId);
          const d = res.data;
          if (!vessel_name && d.vessel_name) vessel_name = d.vessel_name;
          if (!terminal    && d.terminal)    terminal    = d.terminal;
    ...
    } catch { setError('Auto-fill failed. Please fill manually.'); }
  ...
  return (
    <div className="p-6 md:p-8 max-w-4xl mx-auto animate-fade-in">
      <div className="mb-6">
        ...
        <h1 className="text-3xl font-bold text-gray-900 dark:text-white flex items-center gap-2">{isEdit ? '✏️ Edit Vessel Report' : '🚢 New Vessel Report'}</h1>
      ...
      {error && <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-200 px-4 py-3 rounded-xl mb-5 text-sm flex gap-2 hover-lift">⚠️ {error}</div>}
        ...
        <Section title="🚢 Vessel Details">
          <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
            ...
            <button
              ...
              disabled={autoFilling || (form.shore_calc_ids.length === 0 && form.cert_ids.length === 0)}
    ...
    <div className="p-6 md:p-8 max-w-4xl mx-auto animate-fade-in">
      ...
      <div className="flex items-start justify-between gap-4 mb-8 flex-wrap">
        ...
        <div className="flex gap-2 flex-wrap">
          <button onClick={handleDownloadPdf} className="inline-flex items-center gap-2 bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300 border border-green-200 dark:border-green-700 text-sm font-semibold px-4 py-2 rounded-lg hover:bg-green-200 dark:hover:bg-green-900/50 transition hover-lift">
            <Download className="w-4 h-4" /> Download
          </button>
          <button onClick={handlePrintPdf} className="inline-flex items-center gap-2 bg-gray-100 dark:bg-slate-700 text-gray-700 dark:text-gray-200 border border-gray-200 dark:border-gray-600 text-sm font-semibold px-4 py-2 rounded-lg hover:bg-gray-200 dark:hover:bg-slate-600 transition hover-lift">
            <Printer className="w-4 h-4" /> Print
          </button>
          <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-semibold ${report.status === 'final' ? 'bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-200' : 'bg-amber-100 dark:bg-amber-900/30 text-amber-800 dark:text-amber-200'}`}>
            {report.status === 'final' ? <><CheckCircle className="w-4 h-4" /> Final</> : <><Clock className="w-4 h-4" /> Draft</>}
          </span>
          {report.status === 'draft' && (
            <>
              <button onClick={() => navigate(`/vessel-reports/${id}/edit`)} className="bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300 border border-amber-200 dark:border-amber-700 text-sm font-semibold px-4 py-2 rounded-lg hover:bg-amber-200 dark:hover:bg-amber-900/50 transition hover-lift">✏️ Edit</button>
